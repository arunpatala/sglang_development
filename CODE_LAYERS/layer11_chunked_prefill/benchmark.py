"""
Layer 10 — Benchmark: constant-concurrency sliding window.
...
def report(results: list[dict], total_s: float, n: int) -> None:
    ...
    print()
    print("=" * 60)
    print(f" Layer 10 — Batched+Chunked Prefill (page_size=16) Benchmark")
    ...
    md = Path(args.output)
    section = f"""
## Layer 10 — Batched+Chunked Prefill (page_size=16)
