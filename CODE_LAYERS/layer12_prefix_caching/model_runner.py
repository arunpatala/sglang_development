"""
Layer 11 — ModelRunner: prefix-caching aware prefill + decode.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
What changes from Layer 10
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  __init__ accepts enable_prefix_caching=True.  Creates a RadixCache backed
  by the same KVPool.

  prefill_batch additions:
    • For each req with prefix_page_indices: write them into req_to_token
      at columns 0..n_prefix_pages before compute_write_info runs.
      (slot_indices was pre-populated by PrefillAdder.)
    • Before page allocation: if pool is low, call radix_cache.evict().
    • On FINISHED: call radix_cache.cache_finished_req() instead of
      direct kv_pool.free() + req_to_token_pool.free().

  decode_step additions:
    • On FINISHED: same — call radix_cache.cache_finished_req().

  When radix_cache is None (prefix caching disabled), behaviour is
  identical to Layer 10.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
prefill_batch steps
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  1.  Alloc req_pool_idx for brand-new requests.
  2.  Write prefix pages into req_to_token (if prefix_len > 0).
  3.  Evict from RadixCache if KV pool is short on pages.
  4.  compute_write_info for each req → allocates only NEW pages.
  5.  Pack fill_ids → [1, total_tokens] tensor.
  6.  Build position_ids starting at kv_committed_len per request.
  7.  Build qo_indptr, kv_indptr, kv_last_page_lens, kv_indices.
  8.  begin_forward, forward (ExtendKVCtx), end_forward.
  9.  Update kv_committed_len.
  10. For last-chunk reqs: sample first token; RUNNING or FINISHED.
  11. For non-last-chunk reqs: PREFILLING.
"""
