...
Layer 20 changes vs Layer 11:
  • After q/k/v projections (and after o_proj), a masked LoRA delta is added
    when forward_batch.lora_adapter is not None.
  • lora_mask [B, q_len, 1] gates the delta: 1.0 for LoRA tokens, 0.0 for base.
  • The check is a single None guard so non-LoRA batches have zero overhead.
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        if forward_batch.lora_adapter is not None:
            ...
            dq = ada.apply(hidden_states, self.layer_idx, "q_proj")
            dk = ada.apply(hidden_states, self.layer_idx, "k_proj")
            dv = ada.apply(hidden_states, self.layer_idx, "v_proj")
            if dq is not None:
                q = q + dq * mask
            if dk is not None:
                k = k + dk * mask
