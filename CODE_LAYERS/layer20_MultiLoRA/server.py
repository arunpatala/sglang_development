"""
Layer 14 — Server: async FastAPI + asyncio.Future bridge to the Scheduler.
...
The server runs a single target ModelRunner (same as layer 13).
The SpecRunner (draft + target) is constructed separately in spec_runner.py
and used by verify_speculative.py for offline speculative decoding demos.
...
CLI args override individual fields:
    ...
    python server.py --use-gptq                  # GPTQ int4 model stack
    python server.py --kv-memory-fraction 0.7    # reduce pool for tight VRAM
...
def _parse_args() -> dict:
    p = argparse.ArgumentParser(description="Layer 14 inference server")
...
# ── App state ─────────────────────────────────────────────────────────────────
app       = FastAPI(title="Layer 14 — Speculative Decoding")
...
class ChatRequest(BaseModel):
    ...
    lora_id:     Optional[str]  = None   # Layer 20: name of adapter, or None for base model
...
async def chat(req_body: ChatRequest):
    ...
    req = Req(
        ...
        lora_id        = req_body.lora_id,   # Layer 20
        future         = future,
    )
