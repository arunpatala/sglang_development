...
def prefill_position_ids(attention_mask: torch.Tensor) -> torch.Tensor:
    """Per-example position_ids for a left-padded batch (same logic as model_runner)."""
...
def run_lockstep(model, tok, n_steps: int):
    ...
    # Format all prompts through the chat template
    formatted = [
    ...
    for i in range(B):
        kv = KVCache()
        with torch.no_grad():
            logits = model(ind_ids_list[i], attention_mask=ind_mask_list[i], kv_cache=kv)
        ind_kvs.append(kv)
    ...
    for step in range(1, n_steps):
        # Drive both models with the greedy token from B=1 run
        ...
        for i in range(B):
            ...
            with torch.no_grad():
                lg = model(cur, attention_mask=ind_masks[i], kv_cache=ind_kvs[i])
            ind_logits[i] = lg[0, -1, :]

        # Batch B=4 forward with per-example decode position_ids
...
def main():
    n = args.n_compare
    print("=" * 64)
    print(f" verify_batch.py — Layer 4 custom model")
