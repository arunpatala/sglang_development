"""
verify.py — Correctness check for Layer 4A weight loading.

Confirms that our from_pretrained + load_weights produces identical logits
to AutoModelForCausalLM.from_pretrained for the same input sequence.

Runs both models in lockstep for N_COMPARE decode steps:
  - Both receive the SAME token at every step (HF's greedy choice).
...
Since both models use HF's architecture for the forward pass, any difference
is caused by our weight loading rather than a layer implementation bug.
...
def run_lockstep(model_path: str, input_ids: torch.Tensor, n_steps: int):
    ...
    with torch.no_grad():
        ...
        our_logits, _    = our_model(input_ids, attention_mask=attn_mask,
                                     past_key_values=our_kv)
    ...
    for step in range(1, n_steps):
        ...
        with torch.no_grad():
            ...
            our_logits, _    = our_model(current, attention_mask=attn_mask,
                                         past_key_values=our_kv)
...
def main():
    ...
    print("=" * 64)
    print(f" Layer 4A — Weight Loading Verification  ({n_compare} tokens)")
