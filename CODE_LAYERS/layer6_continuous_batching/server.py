...
def _parse_args() -> dict:
    p = argparse.ArgumentParser(description="Layer 6 inference server")
...
# ── App state ─────────────────────────────────────────────────────────────────
app       = FastAPI(title="Layer 6 — Continuous Batching")
...
async def startup():
    ...
    logger.info(f"Starting  model={MODEL_PATH}  port={PORT}  max_running={MAX_RUNNING}")
