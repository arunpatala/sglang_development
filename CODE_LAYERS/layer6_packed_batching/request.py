"""
Layer 5 — Request: per-request state tracked by the scheduler.

Mirrors SGLang's Req class (managers/schedule_batch.py) but stripped to
the essentials needed for continuous batching:

  input_ids     — tokenized prompt
  output_ids    — tokens generated so far
  kv_cache      — this request's PerReqKVCache, populated after prefill
