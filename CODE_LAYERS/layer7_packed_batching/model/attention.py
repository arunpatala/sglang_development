...
Qwen3Attention — multi-head self-attention with:
  ...
  • Backend-object dispatch (SGLang style):
    ...
    ┌─────────────────────────────────────────────────────────────────────┐
    │ config.attn_backend  │ ForwardMode │ Kernel                         │
    │──────────────────────┼─────────────┼────────────────────────────────│
    │ SDPA                 │ PREFILL     │ F.sdpa                         │
    │ SDPA                 │ DECODE      │ F.sdpa (padded BatchedKVCache) │
    │ FLASHINFER           │ PREFILL     │ F.sdpa (B=1, PerReqKVCache)    │
    │ FLASHINFER           │ DECODE      │ FlashInfer ragged (PackedKVCache)│
    └─────────────────────────────────────────────────────────────────────┘

  All dispatch logic lives in backend.py.  This file calls one method:
      self.backend.forward(q, k, v, self.layer_idx, forward_batch)
  Adding a new backend (paged, tensor-parallel) means writing a new class
  in backend.py — this file never changes again.
...
Extensibility surfaces (what SGLang swaps out):
  ...
  • backend → RadixAttention (paged KV, FlashAttn)
...
# ForwardBatch is imported at use-site via backend.py to avoid circular imports.
...
class Qwen3Attention(nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int) -> None:
        ...
        # Backend object owns all kernel dispatch; set once from config.
    ...
    def forward(
        ...
        # ── 4. Backend dispatch ───────────────────────────────────────────
        # All kernel logic (F.sdpa vs FlashInfer, mask building, GQA expand)
        # lives in backend.py.  This call is the only line that varies by backend.
