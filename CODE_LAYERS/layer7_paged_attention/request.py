"""
Layer 7 — Request: per-request state tracked by the scheduler.
...
the essentials needed for continuous batching with paged KV cache:
  ...
  slot_indices  — physical KV pool slot index for each token this request
                  has generated so far (prompt tokens + decode tokens).
                  Replaces PerReqKVCache from layers 5/6.
                  On request finish, all slots are returned to KVPool.free().
...
class Req:
    ...
    # Allocated from KVPool.alloc() at prefill (prompt) and each decode step.
