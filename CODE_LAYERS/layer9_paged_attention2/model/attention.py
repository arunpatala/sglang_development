...
FlashInfer pool tensor convention (page_size=1, NHD layout):
  k_pool[layer]:      [total_slots, n_kv_heads, head_dim]
  Passed as tuple:    k_pool[layer].unsqueeze(1)
                   =  [total_slots, 1, n_kv_heads, head_dim]   ← page_size=1
"""
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None and hasattr(kv_cache, "prefill_slots"):
            ...
            # FlashInfer reads the full KV history (including the just-written
            # new token) from the pool via kv_indices set in begin_forward.
            # Pool tensors: [total_slots, n_kv, head_dim]
            # FlashInfer expects (page_size=1): [total_slots, 1, n_kv, head_dim]
            k_paged = kv_cache.k_pool[self.layer_idx].unsqueeze(1)
            v_paged = kv_cache.v_pool[self.layer_idx].unsqueeze(1)
