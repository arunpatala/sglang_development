"""
Layer 9 — Server: async FastAPI + asyncio.Future bridge to the Scheduler.
...
CLI args override individual fields:
    ...
    python server.py --page-size 32               # larger pages
...
def _parse_args() -> dict:
    p = argparse.ArgumentParser(description="Layer 9 inference server")
...
# ── App state ─────────────────────────────────────────────────────────────────
app       = FastAPI(title="Layer 9 — Paged Attention (ReqToTokenPool + page_size)")
...
async def health():
    ...
    rtp      = scheduler.model_runner.req_to_token_pool
    return {
        "status":                  "ok",
        "running":                 len(scheduler._running),
        "waiting":                 scheduler._waiting.qsize(),
        "kv_pool_free_pages":      pool.available(),
        "kv_pool_total_pages":     pool.total_pages,
        "page_size":               pool.page_size,
        "req_to_token_pool_free":  rtp.available(),
    }
